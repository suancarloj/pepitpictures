...
module.exports = function(socket) {
  ...
  socket.on('live-email-change', (data) => {
    ...
    PicturesService.updateEmail(data.pictureSetId, data.email).then((set) => {
      socket.to(room).emit('live-email-change', data.email);
    });
  });
  socket.on('fetch-fresh-data', (data) => {
    console.log('fetch-fresh-data:', data);
    socket.to(room).emit('fetch-fresh-data', data);
  });
};
