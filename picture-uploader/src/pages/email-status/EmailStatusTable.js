...
const emailStatus = {
  ERROR: (
    <svg xmlns="http://www.w3.org/2000/svg" height="10px" width="10px" viewBox="0 0 576 512">
      <path
...
class EmailStatusTable extends Component {
  ...
  getAllPictureColection = (filter = 'email-only') => {
    return getPictures(null, 200, filter).then((res) => {
  ...
  handleEmailUpdate = (e) => {
    e.preventDefault();
    const { collections, editEmailIndex, email } = this.state;
    const pictureSetId = collections[editEmailIndex]._id;
    setPicturesEmail(pictureSetId, email)
      .then(() => this.getAllPictureColection())
      .then(() => {
        this.setState({ email: '', editEmailIndex: null });
      });
  };
  ...
  render() {
    return (
      <Modal onClickOutside={this.handleCloseModal}>
        ...
        <Container>
          <form method="post" onSubmit={this.filterByEmail}>
            <input
              autoComplete="off"
              type="text"
              ...
              {this.state.collections.map((collection, idx) => {
                ...
                return (
                  <tr className={idx % 2 === 0 ? 'alternate' : ''} key={idx} title={collection._id}>
                    ...
                    <td>
                      {(editEmailActive ? (
                        <form method="post" onSubmit={this.handleEmailUpdate}>
                          <input
                            autoComplete="off"
                            type="text"
                            name="email"
                    ...
                    <td style={{ textAlign: 'center' }}>
                      {emailStatus[collection.emailSent] || '-'}
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      <Button
                        disabled={this.state.publishing}
                        onClick={() =>
                          this.setState({ editEmailIndex: editEmailActive ? null : idx })
                        }
                        type="button"
                      >
